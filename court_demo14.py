...
class HighCourt:
    def __init__(self, root):
        ...
        try:
            bg_image = Image.open("./images/court02.jpg")
            bg_image = bg_image.resize((940, 260), Image.Resampling.LANCZOS)
            bg_image = ctk.CTkImage(light_image=bg_image, size=(940, 260))
        except Exception as e:
            print(f"Could not load background image: {e}")
            bg_image = None

        # Create a frame to act as a container
        ...
        # Create a semi-transparent background frame for text
        self.text_background = ctk.CTkFrame(
            self.subtitle_frame,
            fg_color=("gray85", "gray15"),  # Light and dark mode colors
            bg_color="transparent",
            corner_radius=5
        )
        self.text_background.place(relx=0.5, rely=0.5, anchor="center")

        # Create a label inside the frame for text
        self.subtitle_label = ctk.CTkLabel(
            self.text_background,
            text="Case details",
            font=ctk.CTkFont(size=24, weight="bold"),
            text_color="gray",  # Gray color
            fg_color="transparent",
            wraplength=530,
            padx=20,
            pady=10
    ...
    def create_case_table(self):
        ...
        for col in columns:
            ...
            self.tree.column(col, width=200, anchor="center")  # Center-align cell text

        # Style the table
        style = ttk.Style()
        style.configure("Treeview.Heading", font=('Helvetica', 16, 'bold'), foreground="black", anchor="center")
        style.configure("Treeview", font=('Helvetica', 14, 'bold'), foreground="blue", rowheight=25, anchor="center")

        # # Add scrollbar (optional, uncomment if needed)
