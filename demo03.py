...
class CourtApplication:
    ...
    async def update_text_area(self, message):
    ...
    async def on_button_click(self, language):
        ...
        if not case_number.isdigit():
            await self.update_text_area("Please enter a valid case number.\n")
            return
        ...
        if case_description:
            ...
            await self.update_text_area(translated_message)
            await self.conversation.speak_text(translated_message, language)
        else:
            await self.update_text_area(f"No case found for case number {case_number}.\n")
    ...
    async def manual_voice_input_async(self):
        try:
            ...
            await self.conversation.speak_text(message, lang='pa')
            ...
            if case_number and case_number.isdigit():
                ...
                await self.update_text_area("Invalid input. Please say a valid case number.")
                await self.conversation.speak_text("Please say a valid case number.", lang="en")
        
        except Exception as e:
            print(f"Manual voice input error: {e}")
            await self.update_text_area("Error in voice input.")
    ...
    def create_gui(self):
        ...
        # Move the mic button into its own green-colored frame
        mic_button_frame = tk.Frame(main_container, bg="#228B22")  # Green background for the frame
        ...
        button_frame_bottom = tk.Frame(main_container, bg="#FFFFFF")
        ...
        reset_button = tk.Button(
            ...
            relief="raised",
            bd=4,
            command=self.reset_fields  # Add the reset function to clear entries and text
        )
