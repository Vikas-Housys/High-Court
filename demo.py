...
class CourtApplication:
    ...
    async def manual_voice_input_async(self):
        try:
            self.update_text_area("Listening for case number...")
            case_number = await self.conversation.recognize_speech(language="en-US", timeout=10)
    ...
    def create_gui(self):
        ...
        self.case_number_entry.grid(row=3, column=0, pady=10)

        # Mic button positioned next to case number entry
        ...
        mic_button.grid(row=3, column=1, padx=10, pady=10)  # Placed in the next column (right side)
        ...
        # Text Area
        text_frame = tk.Frame(outer_frame, bg="#87CEEB")
        text_frame.grid(row=5, column=0, columnspan=3, pady=10)

        self.text_area = tk.Text(text_frame, width=45, height=8, wrap=tk.WORD, font=("Arial", 12), bd=2, relief="solid")
